-- Fixed Double Materiality Assessment Schema
-- Removed circular foreign key references
...
CREATE TABLE IF NOT EXISTS topics (
  ...
  description TEXT,
  esrs_code VARCHAR(10),
  organization_id UUID REFERENCES organizations(id) ON DELETE CASCADE,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Stakeholders table (create first)
...
CREATE POLICY "Users can view value chain for their organization" ON value_chain
  FOR SELECT USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can manage value chain for their organization" ON value_chain
  FOR ALL USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can view IROs for their organization" ON iros
  FOR SELECT USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can manage IROs for their organization" ON iros
  FOR ALL USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can view stakeholder feedback for their organization" ON stakeholder_feedback
  FOR SELECT USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can manage stakeholder feedback for their organization" ON stakeholder_feedback
  FOR ALL USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can view scores for their organization" ON scores
  FOR SELECT USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can manage scores for their organization" ON scores
  FOR ALL USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can view evidence for their organization" ON evidence
  FOR SELECT USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can manage evidence for their organization" ON evidence
  FOR ALL USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can view audit log for their organization" ON audit_log
  FOR SELECT USING (organization_id IN (
    SELECT id FROM organizations WHERE id IN (
      SELECT organization_id FROM profiles WHERE id = auth.uid()
    )
