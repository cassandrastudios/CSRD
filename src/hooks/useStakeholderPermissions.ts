...
import { Stakeholder, StakeholderRole, STAKEHOLDER_PERMISSIONS } from '../types/stakeholder'
...
export function useStakeholderPermissions(organizationId: string) {
  ...
  const [permissions, setPermissions] = useState(STAKEHOLDER_PERMISSIONS.contributor)
  ...
  const canAccess = (areaType: string, areaId?: string) => {
    ...
    return stakeholder.assigned_areas.some(area => area.startsWith(`${areaType}:`))
  }
  ...
  const canManage = (targetRole: StakeholderRole) => {
    ...
    if (stakeholder?.role === 'manager' && ['manager', 'contributor'].includes(targetRole)) {
      return true
    }
  ...
  const unassignArea = async (areaType: string, areaId: string) => {
    ...
    try {
      ...
      const updatedAreas = stakeholder.assigned_areas.filter(area => area !== areaToRemove)
