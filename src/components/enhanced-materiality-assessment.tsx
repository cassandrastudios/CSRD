...
export function EnhancedMaterialityAssessment() {
  ...
  const fetchData = async () => {
    try {
      const [
        ...
        supabase.from('esrs_topics').select('*').eq('is_active', true).order('code'),
  ...
  const updateAssessment = useCallback((topicId: number, insideOut: number, outsideIn: number) => {
    if (debouncedUpdate.current) {
      clearTimeout(debouncedUpdate.current)
    }
        ...
        return prev.map(a => 
          a.esrs_topic_id === topicId 
            ? { 
                ...
                materiality_status: calculateMaterialityStatus(insideOut, outsideIn)
              }
            : a
        )
      } else {
  ...
  const calculateMaterialityStatus = (insideOut: number, outsideIn: number): 'not_material' | 'material' | 'highly_material' => {
  ...
  // Calculate progress
  const progress = topics.length > 0 ? (assessments.length / topics.length) * 100 : 0
  ...
  const materialityData = assessments.reduce((acc, assessment) => {
    const status = assessment.materiality_status
    acc[status] = (acc[status] || 0) + 1
    return acc
  }, {} as Record<string, number>)

  const materialityChartData = Object.entries(materialityData).map(([status, count]) => ({
    status: status.replace('_', ' ').toUpperCase(),
    count,
    color: status === 'highly_material' ? '#ef4444' : status === 'material' ? '#f59e0b' : '#10b981'
  }))
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Comprehensive ESRS materiality assessment with stakeholder engagement and evidence collection
            </p>
          </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600">{assessments.length}</div>
                <div className="text-sm text-gray-500">Topics Assessed</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-orange-600">
                  {assessments.filter(a => a.materiality_status === 'material' || a.materiality_status === 'highly_material').length}
                </div>
                <div className="text-sm text-gray-500">Material Topics</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-purple-600">{stakeholders.length}</div>
              ...
              <CardContent className="p-0">
                <div className="max-h-96 overflow-y-auto">
                  {filteredTopics.map(topic => {
                    ...
                    const isMaterial = assessment?.materiality_status === 'material' || assessment?.materiality_status === 'highly_material'
                    
                    return (
                      <div
                        key={topic.id}
                        className={`p-4 border-b cursor-pointer hover:bg-gray-50 ${
                          selectedTopic?.id === topic.id ? 'bg-blue-50 border-blue-200' : ''
                        }`}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            {assessment && (
                              <Badge variant={isMaterial ? 'default' : 'secondary'}>
                                {assessment.materiality_status.replace('_', ' ')}
                              </Badge>
                            )}
                            {assessment ? (
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>{selectedTopic.code} - {selectedTopic.category}</CardDescription>
                    </div>
                  ...
                  <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <TabsContent value="overview" className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-600">
                          <p><strong>High Impact Sectors:</strong> {selectedTopic.industry_benchmarks.high_impact_sectors.join(', ')}</p>
                          <p><strong>Benchmark Threshold:</strong> {selectedTopic.industry_benchmarks.benchmark_threshold}/5</p>
                        </div>
                      </div>
                    </TabsContent>
                    ...
                    <TabsContent value="scoring" className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          <h3 className="font-medium mb-4">Inside-Out Impact</h3>
                          <p className="text-sm text-gray-600 mb-4">
                            How your organization affects the environment, people, and society
                          </p>
                          <div className="space-y-2">
                            <label className="text-sm">Impact Level: {getAssessment(selectedTopic.id)?.inside_out_impact || 1}</label>
                            <input
                              type="range"
                              min="1"
                              max="5"
                              value={getAssessment(selectedTopic.id)?.inside_out_impact || 1}
                              onChange={(e) => updateAssessment(selectedTopic.id, parseInt(e.target.value), getAssessment(selectedTopic.id)?.outside_in_impact || 1)}
                              className="w-full"
                            />
                        ...
                        <div>
                          <h3 className="font-medium mb-4">Outside-In Impact</h3>
                          <p className="text-sm text-gray-600 mb-4">
                            How sustainability issues affect your organization financially and operationally
                          </p>
                          <div className="space-y-2">
                            <label className="text-sm">Impact Level: {getAssessment(selectedTopic.id)?.outside_in_impact || 1}</label>
                            <input
                              type="range"
                              min="1"
                              max="5"
                              value={getAssessment(selectedTopic.id)?.outside_in_impact || 1}
                              onChange={(e) => updateAssessment(selectedTopic.id, getAssessment(selectedTopic.id)?.inside_out_impact || 1, parseInt(e.target.value))}
                              className="w-full"
                            />
                            <div className="flex justify-between text-xs text-gray-500">
                              <span>Low (1)</span>
                            ...
                            <ScatterChart data={scatterData}>
                              ...
                              <Tooltip 
                                formatter={(value, name, props) => [
                                  value, 
                                  name === 'insideOut' ? 'Inside-Out Impact' : 'Outside-In Impact'
                                ]}
                              ...
                              <Scatter dataKey="insideOut" fill="#8884d8">
                                {scatterData.map((entry, index) => (
                                  <Cell 
                                    key={`cell-${index}`} 
                                    fill={
                                      entry.materiality === 'highly_material' ? '#ef4444' :
                                      entry.materiality === 'material' ? '#f59e0b' : '#10b981'
                                    } 
                                  />
                                ))}
                              </Scatter>
                    ...
                    <TabsContent value="stakeholders" className="space-y-4">
                      ...
                      <div className="space-y-2">
                        {getTopicEngagements(selectedTopic.id).map(engagement => (
                          <div key={engagement.id} className="p-3 border rounded-lg">
                            <div className="flex justify-between items-start">
                              <div>
                                <div className="font-medium">{engagement.engagement_type}</div>
                                <div className="text-sm text-gray-500">{engagement.activity_date}</div>
                                {engagement.description && (
                                  <div className="text-sm mt-1">{engagement.description}</div>
                                )}
                              </div>
                              <div className="flex items-center gap-2">
                                <Badge variant="outline">
                                  Influence: {engagement.influence_on_scoring}/5
                                </Badge>
                    ...
                    <TabsContent value="reports" className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Card>
                          <CardHeader>
                            <CardTitle className="text-lg">Materiality Status</CardTitle>
                          </CardHeader>
                          <CardContent>
                            <div className="h-32">
                              <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                  <Pie
                                    ...
                                    dataKey="count"
                                  >
                                    {materialityChartData.map((entry, index) => (
              ...
              <Card>
                <CardContent className="flex items-center justify-center h-64">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500">Choose an ESRS topic from the list to view details and begin assessment</p>
                  </div>
                </CardContent>
              </Card>
            )}
