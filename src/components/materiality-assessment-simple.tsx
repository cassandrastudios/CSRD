...
export function MaterialityAssessmentSimple() {
  ...
  const [selectedTopic, setSelectedTopic] = useState<SimpleESRSTopic | null>(null)
  ...
  const updateAssessment = useCallback((topicId: number, impact: number, financial: number) => {
    if (debouncedUpdate.current) {
      clearTimeout(debouncedUpdate.current)
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-3xl font-bold text-orange-600">
                  {assessments.filter(a => a.impact_materiality >= 3 || a.financial_materiality >= 3).length}
                </div>
              ...
              <CardContent className="p-0">
                <div className="max-h-96 overflow-y-auto">
                  {filteredTopics.map(topic => {
                    ...
                    const isMaterial = assessment && (assessment.impact_materiality >= 3 || assessment.financial_materiality >= 3)
                  ...
                  <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <TabsContent value="scoring" className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2">
                            <label className="text-sm">Impact Level: {getAssessment(selectedTopic.id)?.impact_materiality || 1}</label>
                            <input
                              type="range"
                              min="1"
                              max="5"
                              value={getAssessment(selectedTopic.id)?.impact_materiality || 1}
                              onChange={(e) => updateAssessment(selectedTopic.id, parseInt(e.target.value), getAssessment(selectedTopic.id)?.financial_materiality || 1)}
                              className="w-full"
                            />
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            <label className="text-sm">Financial Level: {getAssessment(selectedTopic.id)?.financial_materiality || 1}</label>
                            <input
                              type="range"
                              min="1"
                              max="5"
                              value={getAssessment(selectedTopic.id)?.financial_materiality || 1}
                              onChange={(e) => updateAssessment(selectedTopic.id, getAssessment(selectedTopic.id)?.impact_materiality || 1, parseInt(e.target.value))}
                              className="w-full"
                            />
                            <div className="flex justify-between text-xs text-gray-500">
                              <span>Low (1)</span>
                              ...
                              <Scatter dataKey="impact" fill="#8884d8">
                                {scatterData.map((entry, index) => (
                                  <Cell 
                                    key={`cell-${index}`} 
                                    fill={
                                      (entry.impact >= 4 || entry.financial >= 4) ? '#ef4444' :
                                      (entry.impact >= 3 || entry.financial >= 3) ? '#f59e0b' : '#10b981'
                                    } 
                                  />
                                ))}
                              </Scatter>
                    ...
                    <TabsContent value="reports" className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <Card>
                          <CardHeader>
                            <CardTitle className="text-lg">Materiality Distribution</CardTitle>
                          </CardHeader>
                          <CardContent>
                            <div className="space-y-2">
