...
export function ComplianceCheck() {
        ...
        prev.map(item => {
          ...
          switch (item.id) {
            case 'materiality-assessment':
              status =
                materialityData && materialityData.length > 0
                  ? 'completed'
                  : 'not_started';
              break;
            case 'esg-data-collection':
              const completedMetrics =
                metricsData?.filter(m => m.status === 'completed').length || 0;
  ...
  const overallProgress =
    requiredTotal > 0 ? (requiredCompleted / requiredTotal) * 100 : 0;
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {categoryCompleted} of {categoryItems.length} items
                      completed
                    </CardDescription>
                  </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex justify-between items-center">
              ...
              <Button onClick={checkComplianceStatus}>Refresh Status</Button>
            </div>
          </CardContent>
        </Card>
      </div>
